...
__thread EventLoop* t_loopInThisThread = 0; // 当前线程EventLoop指针

const int kPollTimeMs = 10000;  // Poller等待事件

int createEventfd()             // 创建唤醒线程的文件描述符
{
...
class IgnoreSigPipe             // 利用构造函数忽略SIGPIPE信号
{
public:
    IgnoreSigPipe()
    {
    ...
    currentActiveChannel_(NULL)
{
    ...
    wakeupChannel_->setReadCallback(boost::bind(&EventLoop::handleRead, this));     // wakeupChannel_回调函数, 用于唤醒Poller
...
void EventLoop::quit()
{
    quit_ = true;       // quit_是bool类型, 原子性操作, 跨线程调用时, 该线程可能阻塞在poller_或handleEvent位置, 此时需唤醒操作
    if (!isInLoopThread())
    {
        wakeup();       // 如果不是在当前线程调用quit, 先唤醒该线程, 因为当前线程可能阻塞在Poller或正在处理事件函数
    }
}
...
muduo没有反复执行doPendingFunctors()直到pendingFunctors为空，这是有意的，否则IO线程可能陷入死循环(doPendingFunctors中functors又调用queueInLoop将任务添加进来)，无法处理IO事件。 交换的任务执行完毕就可以
