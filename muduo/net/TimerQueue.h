...
namespace muduo
{
// 定时器的具体实现, 一个定时器只能定义一个时间, 
// 通过把多个定时时间按顺序排列, 每次定时器都定第一个时间, 从而实现多个定时器
///
...
private:
    ...
    //std::map<Timestamp, Timer*>           TimerList;      // 没有这么用是因为可能多个Timer对象对应同一个Timerstamp
    typedef std::pair<Timestamp, Timer*>    Entry;
    typedef std::set<Entry>                 TimerList;      // 按时间排序
    typedef std::pair<Timer*, int64_t>      ActiveTimer;
    typedef std::set<ActiveTimer>           ActiveTimerSet; // 按地址排序, TimerList与ActiveTimerSet相同, 一个是按时间戳排序一个按地址排序
    ...
    bool insert(Timer* timer);          // 插入定时器, 返回值用于判断是否调整定时器列表

    EventLoop* loop_;                   // 所属EventLoop
    const int timerfd_;                 // timerfd_create所创建的文件描述符
    Channel timerfdChannel_;            // 定时器通道, 当定时器事件到来时, 可读事件产生, 会回调handleRead函数, 这里setReadCallback并没有绑定void(Timestamp)类型函数???
