...
void muduo::net::defaultConnectionCallback(const TcpConnectionPtr& conn)    // 默认连接到来回调函数
{
...
void muduo::net::defaultMessageCallback(const TcpConnectionPtr&,            // 默认消息到来回调函数
                                        Buffer* buf,
                                        Timestamp)
{
    buf->retrieveAll();
...
TcpConnection::TcpConnection(EventLoop* loop,
    ...
    : loop_(CHECK_NOTNULL(loop)),   // 检查loop是否为空
    name_(nameArg),
    state_(kConnecting),
...
void TcpConnection::shutdown()              // 如果缓冲区还有数据写入, 则关闭操作只改变状态没有其余操作
{
