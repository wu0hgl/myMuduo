...
// 清除定时器, 读取文件描述符, 避免一直触发(muduo中EPollPoller采用电平触发, 防止busy loop
...
{
    /* 这里setReadCallback并没有绑定void(Timestamp)类型函数??? */
                             ...
                             double interval)    // 间隔时间
{
    ...
    // 即使不是在当前线程中调用addTimer也会添加到loop_所对应的IO线程中处理
...
// rvo优化, 不会返回对象时不会再拷贝构造std::vector<Entry>
...
bool TimerQueue::insert(Timer* timer)
{
    ...
    Timestamp when = timer->expiration();               // 获取timer的超时时间
